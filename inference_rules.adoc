...
division allows calibration for quantization or pruning, and does not allow any retraining.
...
* A calibration data set, to be used for quantization (see quantization
  section) or pruning (see pruning section), that is a small subset 
  of the training data set used to generate the weights
...
=== Weight Definition, Quantization, and Pruning
...
* Discarding non-zero weight elements, including structured pruning such as
  channel slimming, attention head removal, and layer dropping
