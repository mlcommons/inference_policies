...
The Network division inherits all requirements from the Closed division and imposes further constraints and supports only the Datacenter suite.

In the Network division the SUT is connected to the loadgen over a network fabric. The Query Dispatch Library (QDL) component which is the client-side SUT proxy which connects the Loadgen to an SUT over a network. The QDL is a submitter implemented component.
...
MLPerf distinguishes between fabric interconnects and bus interconnects. Fabrics interconnects are required and bus interconnects are forbidden.
...
1. Connect systems in meters to kilometers distances
...
Fabric and protocol must be reported in the submission metadata. Submission metadata must be include sufficient to determine OSI layers one through four of the submission's network stack.

==== SUT over the Network and Setup Constraints

* SUT parameters and configuration must be specified in detail in the submission results. 
* Network interface characteristics must be added to the report, both at the SUT side and at the loadgen server side, including networking components or any other hardware in the system that either stands between them or are involved in any way in the test.

==== Benchmarks and QSL Preprocessing

Current benchmark scenarios remain the same but tested over the network. As specified below the data formats for inputs and outputs can be adapted and preprocessed for network transmission, providing a tradeoff between compute and network bandwidth. The preprocessing must be selected only from the options at the table below. The selected option can be defined by the submitter according to the specific network link and acceleration implemented. The preparation of the data is done in the QSL in an untimed manner. The details for the allowed preprocessing options for each benchmark are specified in the table below.
...
. Compression scheme need pre-approval at least two weeks before a submission deadline.
. A compression scheme may use information from the training set, but not the not validation set (ex: check index probability).
