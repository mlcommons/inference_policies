...
* Everything outside the LoadGen node should be considered as part of the SUT, for instance for counting power and latency. As an example, components outside the nodes like switch or load balancer should be considered part of the SUT. 
...
* It is required to add a system diagram that shows how the components between the LoadGen node and the SUT node are connected and to add text that explains the diagram.
* For reproducibility, it is required to specify software version of all components, Hardware configurations, software stacks, dockers, and settings of all components and stacks.
