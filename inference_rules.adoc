...
|Language |GPT-OSS-120B| AIME25, GPQA Diamond, LiveCodeBench v6| __num_inputs*max_seq_len*dtype_size__ | __15330*dtype_size__ | __throughput*15330*dtype_size__
...
|Commerce |DLRMv3 | Synthetic Streaming 100B Dataset |__(2 + 6*uih_seq_len + 6*num_candidates)*int64_size__footnote:[Each DLRMv3 request contains UIH features (user_id, item_id, ratings, timestamps, weights, category_id for each history item) and candidate features (6 features per candidate). All values are int64.] |__TBD__footnote:[DLRMv3 ingress bandwidth TBD - depends on average UIH sequence length and number of candidates per request.] | __throughput*TBD__
