...
* The trace generated by LoadGen must be stored in the DRAM that most faithfully simulates queries arriving 
  ...
  the network or I/O device. Minimum bandwidths for each benchmark can be found in <<appendix-bw>>. From 2.1, all components mentioned in the system architecture must be present in the system during the run. In 2.0 it is sufficient for the system architecture description to demonstrate that the system could be populated with additional NICs, and that their addition would provide sufficient bandwidth to be delivered to and from the accelerator. A system architecture description must be provided along with the submission.  This write-up must include:
