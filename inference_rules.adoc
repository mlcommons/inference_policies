...
In the Network division the SUT is connected to the loadgen over a network fabric. The loadgen architecture is extended by introducing a Query Dispatch Library (QDL) component which is the client-side SUT proxy. The QDL is a submitter implementation component. The QDL implements the SUT API. The Network division supports only the Datacenter suite.
...
1. Connects systems in meters to kilometers distances
