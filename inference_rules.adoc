...
====== Networking
Since the constraints for a Closed division datacenter submission (described below) require both offline and server scenario submissions, a datacenter system must have at least as much networking ingress bandwidth to the system as is required to sustain its Server scenario performance for all its submissions.  This can be in the form of integrated networking built into the accelerator(s) themselves or in the form of dedicated NICs and must be described in the system JSON as mentioned in the general submission rules (https://github.com/mlcommons/policies/blob/master/submission_rules.adoc).  All necessary networking as mentioned here must be populated if power is measured along with performance. 
Additionally, systems running with start_from_device enabled should have adequate bandwidth between the networking and the accelerator. 
...
and have Offline results inferred according to the following rules:

- The corresponding Offline result is 1000 divided by the Mean latency in
  milliseconds.  For example, a Single Stream mean latency of 20 milliseconds
would imply an Offline result of 50 samples per second. The inferred Offline
accuracy will be the same as the Single Stream accuracy.
...
and `measurements/`, calling them `offline` for the Offline scenario.
