...
If the selected "duration" or "samples/query" requires loading multiple copies of the test dataset, submitters are `NOT` allowed to sort samples beyond the boundary of a dataset, including if "duration" or "samples/query" meets the minimum requirement, for all LLM workloads. 
...
A: Yes. However, if the selected duration or number of queries requires loading multiple copies of the dataset, submitters are not allowed to sort samples beyond the boundary of a dataset, including if the duration and number of queries is set to the the minimum requirement, for all LLM workloads. 
