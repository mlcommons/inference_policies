...
CLOSED: There are two benchmark suites, one for Datacenter systems and one for Edge (defined herein as non-datacenter) systems. The suites share multiple benchmarks, but characterize them with different requirement. Read the specifications carefully.

The Datacenter benchmark suite is as follows:
...
|Vision |Medical image segementation |3D UNET |?? |?? |99% of FP32 and 99.9% of FP32 (??) |N/A  
...
The Edge benchmarks *require* the following scenarios, and sometimes permit an optional scenario:
...
|Vision |Medical image segementation |Single Stream, Offline | N/A
...
=== DLRM
Q: What's the distribution for user-item pairs in the Server Scenario?
A: In the case of DLRM we have agreed that we should use multiple samples drawn from a distribution, similar to the one shown on Fig. 5: "Queries for personalized recommendation models" in the [DeepRecSys](https://arxiv.org/abs/2001.02772) paper.
