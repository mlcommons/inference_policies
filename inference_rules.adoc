...
|Multistream | Loadgen sends next query, containing 8 samples, as soon as SUT completes the previous query | 270,336 queries and 600 seconds | None | 99% | 99%-ile measured latency|
|===
...
if the multistream result is 200ms, the inferred offline result is 40 img/s. The inferred accuracy will be the same as
multi-stream accuracy.

- an offline result inferred from a multistream result is 1000 divided by the mean latency in milliseconds. For example,
if the multistream result is 25ms, the inferred offline result is 40 img/s. The inferred accuracy will be the same as
multi-stream accuracy.
...
create copies of the corresponding Single Stream directories under `results/`
...
* Multistream: LoadGen measures 99th percentil latency using a single test run. For
the test run, LoadGen sends an initial query then continually sends the next
query as soon as the previous query is processed.
