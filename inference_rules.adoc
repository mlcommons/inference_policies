...
* The QDL should serially get the names from all the Nodes of the SUT, aggregate the response in the Name Method after getting response from all Nodes. Each Node must have a unique Name.   
...
The SUT and QDL must run on two physically separated and distinct systems. The SUT can contain multiple Nodes.
...
* Everything outside the LoadGen node should be considered as part of the SUT, for instance for counting power and latency. As an example, components outside the nodes like switch or load balancer should be considered part of the SUT. The SUT can contain multiple Nodes.
...
* Network interface characteristics must be added to the report, both at the SUT side, for every Node, and at the LoadGen system side, including networking components, and name any other components through which data passes between LoadGen and the SUT for reproducibility.
* It is required to add a system diagram that shows how the components between the LoadGen node and the SUT nodes are connected and to add text that explains the diagram.
