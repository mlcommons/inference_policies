...
|3D UNET	        |one image
|BERT	            |one sequence
|DLRMv3	            |up to 700 user-item pairs (more details in FAQ)
|Wan2.2-T2V-A14B-Diffusers            |A pair of postive and negative prompts
...
|Language |Reasoning |GPT-OSS |mlperf_gpt_oss_performance mlperf_gpt_oss_accuracy | perf: 6396, acc: 4395 | 99% of mxfp4 reference (83.13 * 0.99) on mlperf_gpt_oss_accuracy dataset | TTFT/TPOTfootnote:[For GPT-OSS, 2 latency metrics are collected - time to first token (TTFT) which measures the latency of the first token, and time per output token (TPOT) which measures the average interval between all the tokens generated.]: 3000ms/ 80ms 
|Commerce |Recommendation |DLRMv3 |Synthetic Dataset | 34996 |99.9% of the original three BF16 metrics: NE (86.69%), Accuracy (69.65%), and AUC (78.66%) | 80 ms
|Generative |Text to video |Wan2.2-T2V-A14B-Diffusers |VBench | 248 |99% of Reference (70.48%) | N/A
...
|Multimodal |Image to text |VLM |Shopify-product-catalogue | 48289 |99% of reference F1 0.7903 | 12 s
|===
...
|Generative |Text to video |SingleStream, Offline
|Graph |Node classification |Offline
|Audio |Speech to Text | Offline
|Multimodal |Image to text | Server, Offline
|===
...
|Vision |Object detection |Yolo |COCO safe subset | 1525 | 99% of FP32 (0.52866 mAP)
...
No compression allowed.
|Language | Reasoning | GPT-OSS | Input is either Token IDs, Input Masks and Input Lengths or just the Token IDs (the other tensors are generated at the SUT in a timed operation).
...
|Generative | Text to video | Wan2.2-T2V-A14B-Diffusers | Allow any lossless compression that will be suitable for production use
|Multimodal | Image to text | VLM | No compression allowed.
...
* BERT, Llama2-70b, Llama3.1-405B, Mixtral-8x7B, Llama3.1-8B, GPT-OSS and DeepSeek-r1 take texts

* RNN-T and Whisper takes a waveform

* DLRMv3 takes a variable sized set of items, each a sequence of embedding indices
...
* Techniques that only improve performance by artificially creating batches of identical samples. Sorting samples beyond the boundary of a dataset is prohibited for all LLM workloads such as - Llama2-70b, Llama2-70b-Interactive, Llama3.1-405B, DeepSeek-r1, Llama3.1-8B, Mixtral-8x7B and GPT-OSS.
...
A: The algorithms used by the benchmarks (greedy search and beam search) are described at a high level here: https://huggingface.co/blog/how-to-generate. Specifically, Llama2-70b, Llama3.1-405B, Mixtral-8x7B, DeepSeek-r1, Llama3.1-8B and GPT-OSS use greedy search.
...
Datacenter systems must provide at least the following bandwidths from the output location (e.g. DRAM) to the network or I/O device. The minimum bandwidth is a function of the throughput achieved by the SUT and the output data types. For all models except 3D Unet, SDXL and Wan2.2-T2V-A14B-Diffusers, the output sizes are negligible. Therefore, for those models, the egress bandwidth must simply be greater than 0.
