...
* Speculative decoding for auto-generative language models (i.e. using a smaller model to predict the next token for the reference model), with the exception of certain LLM workloads including - DeepSeek-r1-Interactive, as prescribed in <<appendix-speculative-decoding>>
...
All Implementations must use the reference MTP (Multi-Token Prediction) Head with the reference speculative-decoding algorithm and configuration.

Implementations that artificially manipulate acceptance rates are disallowed, eg: quantization of the MTP head weights.

|===
|Benchmark |Scenario |Speculative Decoding Algorithm |Configuration

|DeepSeek-r1
|Interactive
|EAGLE-style decoding with DeepSeek-r1 MTP head
