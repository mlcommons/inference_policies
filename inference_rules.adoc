...
|3D UNET	        |one image
|BERT	            |one sequence
|DLRMv3	            |one request (containing one user and up to 100 candidates)
|WAN-2.2-T2V-A14B	            |one text prompt
...
|QWen3-VL-235B-A22B            |one image and text prompt
|Whisper	            |one audio
|===
...
|Language |Text Generation (Question Answering, Math and Code Generation) |GPT-OSS-120B |AIME25, GPQA Diamond, LiveCodeBench v6 | 6396 | TBD (Final score based on weighted dataset accuracy)footnote:[GPT-OSS-120B evaluation uses three datasets with different repeat counts: AIME25 (8 repeats), GPQA Diamond (5 repeats), LiveCodeBench v6 (3 repeats). Final score = sum(dataset_correct / dataset_repeats).]| TTFT/TPOTfootnote:[For GPT-OSS-120B, 2 latency metrics are collected - time to first token (TTFT) which measures the latency of the first token, and time per output token (TPOT) which measures the average interval between all the tokens generated.]: 3000 ms/80 ms
|Vision |Visual Language Model |QWen3-VL-235B-A22B |Shopify product-catalogue | 48289 | 99% of BF16 (Category Hierarchical F1 Score >= 0.7824)| 12 s (99th percentile)
...
|Generative |Text to video |Offline
...
* Techniques that only improve performance by artificially creating batches of identical samples. Sorting samples beyond the boundary of a dataset is prohibited for all LLM workloads such as - Llama2-70b, Llama2-70b-Interactive, Llama3.1-405B, DeepSeek-r1, GPT-OSS-120B, Llama3.1-8B and Mixtral-8x7B.
...
=== DLRMv3
Q: What is the input format for DLRMv3?

A: DLRMv3 uses a synthetic streaming dataset with sequential recommendation characteristics. Each request contains one user and up to 100 candidates. The dataset includes 5 million users interacting with a billion items over 100 timestamps, with 1% of the dataset used for inference benchmarking.

Q: What are the accuracy metrics for DLRMv3?

A: DLRMv3 uses three metrics: Normalized Entropy (NE), Accuracy, and AUC. For accepted submissions, all three metrics must be within 99.9% of the reference implementation values: NE=86.687%, Accuracy=69.651%, AUC=78.663%.

Q: How do I download the DLRMv3 dataset?

A: The sampled DLRMv3 dataset and trained checkpoint are available at https://inference.mlcommons-storage.org/. You can also use MLCFlow automation: `mlcr get-dataset-mlperf-inference-dlrmv3-synthetic-streaming,_mlc,_r2-downloader --outdirname=<path_to_download> -j`
...
A: The algorithms used by the benchmarks (greedy search and beam search) are described at a high level here: https://huggingface.co/blog/how-to-generate. Specifically, Llama2-70b, Llama3.1-405B, Mixtral-8x7B, DeepSeek-r1, GPT-OSS-120B, and Llama3.1-8B use greedy search.
...
|Language |GPT-OSS-120B| AIME25, GPQA Diamond, LiveCodeBench v6| __num_inputs*max_seq_len*dtype_size__ | __TBD*dtype_size__footnote:[GPT-OSS-120B max_seq_len varies by dataset. TBD - to be determined based on final dataset specifications.] | __throughput*TBD*dtype_size__
|Vision |QWen3-VL-235B-A22B| Shopify product-catalogue| __num_inputs*(image_size+text_len)*dtype_size__ | __TBD*dtype_size__footnote:[QWen3-VL-235B-A22B input size varies based on image dimensions and text prompt length. TBD - to be determined based on final specifications.] | __throughput*TBD*dtype_size__
|Commerce |DLRMv3 | Synthetic Streaming 100B Dataset |__avg(num_candidates_per_request)*embedding_size*dtype_size__footnote:[Each DLRMv3 request contains one user and up to 100 candidates.] |__TBD*dtype_size__footnote:[DLRMv3 ingress bandwidth calculation TBD - to be determined based on final embedding dimensions and average candidates per request.] | __throughput*TBD*dtype_size__
|Generative |WAN-2.2-T2V-A14B |VBench prompts (max_prompt_len=TBD) | __num_inputs*max_prompt_len*dtype_size__ | __TBD*dtype_size__footnote:[WAN-2.2-T2V-A14B prompt length TBD - to be determined based on VBench prompts specifications.] |  __throughput*TBD*dtype_size__
|Graph |RGAT |IGBH | negligible | negligible | __> 0__
|===
=== Egress Bandwidth

Datacenter systems must provide at least the following bandwidths from the output location (e.g. DRAM) to the network or I/O device. The minimum bandwidth is a function of the throughput achieved by the SUT and the output data types. For all models except 3D Unet, WAN-2.2-T2V-A14B, and LLMs (Llama2-70b, Llama3.1-405B, Mixtral-8x7B, DeepSeek-r1, GPT-OSS-120B), the output sizes are negligible. Therefore, for those models, the egress bandwidth must simply be greater than 0.
...
|Language |DeepSeek-r1| mlperf_deepseek_r1| __num_inputs*max_seq_len*dtype_size__ | __20000*dtype_size__ | __throughput*20000*dtype_size__
...
|Vision |QWen3-VL-235B-A22B| Shopify product-catalogue| negligible | negligible | __> 0__
