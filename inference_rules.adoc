...
* The QDL should serially get the names from all the Nodes of the SUT and aggregate the responses in the Name Method after getting response from all Nodes. Each Node must have a unique Name.   
...
* Everything outside the LoadGen node should be considered as part of the SUT, for instance for counting power and latency. As an example, components outside the nodes like a switch or load balancer should be considered part of the SUT. The SUT can contain multiple Nodes.
...
* Network interface characteristics must be added to the report, both at the SUT side for every Node, and at the LoadGen system side, including networking components.  Every other component through which data passes between LoadGen and the SUT must also be described for reproducibility purposes.
