...
A: The benchmark uses the beam search algorithm described at a high level here: https://huggingface.co/blog/how-to-generate#beam-search. Specifically, we use a beam width of 4 and enable early termination. TODO(thomas,bhadri): Add more details here of the beam search in GPT-J.

Q: MLPerf disallows caching queries. Is using a KV-cache in decoding allowed?

A: Using a KV-cache is allowed in the same way as included in the reference model where it does not apply across queries. A KV-cache row is a tensor that must deal with the execution of a single inference query. Every input query must be computed in its entirety. Entries of the KV-cache must be treated similar to activations of a forward pass. They can be quantized with respect to the quantization rules. Similarly, they cannot be pruned (or sparsified).

Q: What are the rules related to KV-cache and query batching?

A: The size of the KV-cache depends on the batch size. Following the rule for allowing caching of sizes and shapes, the KV-cache size can also be cached across queries. Other than batching and quantization rules (that apply to activations), alternative attention mechanisms (such as paged, multi-query, sparse, group query attention, etc) or wholesale replacement of the reference KV-cache execution are disallowed.
