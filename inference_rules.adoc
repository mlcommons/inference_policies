...
====== Networking
A datacenter system must be equipped with all the necessary networking required by the system architecture described in the Section 5.1: LoadGen Operation.  The details of the networking components must be described in the appropriate field of the System JSON (https://github.com/mlcommons/policies/blob/master/submission_rules.adoc).  All necessary networking must be populated if power is measured along with performance. 
...
* The trace generated by LoadGen must be stored in the DRAM that most faithfully simulates queries arriving 
  ...
  the network or I/O device.  All components mentioned in the system architecture description must be populated during the run. 
