...
division allows calibration for quantization or pruning to adjust weights.
...
* A calibration data set, to be used for quantization or pruning (see calibration
  section), that is a small subset of the training data set used to generate the weights.
...
=== Weight Definition and Calibration
...
Submitters may do arbitrary purely mathematical, reproducible quantization or
pruning (setting values to zero) using only the calibration data, weight, and bias 
tensors from the benchmark owner provided model to any numerical format
that achieves the desired quality. The calibration method must be publicly
...
* Masking or updating the values of weights based on calibration set
...
* Retraining weights based on gradient-based techniques using the calibration
  data (or any additional data)

* Wholesale weight supplements

* Discarding non-zero weight elements in a structural way such that entire rows 
  or columns are removed, such as channel slimming, attention head removal, and 
  layer dropping
