...
* All submitters are ordered (alphabetically, or any other way) and are assigned a number according to their position starting with 1

* A random generator that reorders (shuffled) the list is used live during a review meeting. List Randomizer from random.org can be used.

* Peer Reviews are assigned based on the new order with the last submitter in the new order assigned to review the first one. For example, with companies Company1, Company2, and Company3 participating, their assigned numbers will be 1, 2, and 3, respectively. The list gets reordered 2, 1, 3. The Assigned Reviews will be: Company2 will review Company1, Company1 will review Company3, Company3 will review Company2. 

* Chair will open an github issue against each company that has review assignment. Issues will be closed once reviewers indicate tha they finished their tasks. 

* During the review, reviwers are asked to pay special attention to: (1) results validity, (2) methodology, (3) instructions for reproducibility, and (4) content of json files in systems directory.

* Any issues discovered should be filed as github issues and resolved as usual.
